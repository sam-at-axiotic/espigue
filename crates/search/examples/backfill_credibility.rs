...
//!   cargo run -p search --example backfill_credibility [-- DB_PATH [LIMIT]]
//! ```
//!
...
use search::{update_paper_credibility, S2Client};
...
enum Batch {
    ...
    Ok(Vec<Option<search::S2PaperFull>>),
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    search::lit_migrate(&pool, 1024).await?;
