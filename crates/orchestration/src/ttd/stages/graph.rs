            ...
            join_set.spawn(async move {
                ...
                let _permit = match sem.acquire().await {
                    ...
                    Err(_) => {
                        return (
                            expert_id,
                            Err(TtdError::Executor(base::error::AlzinaError::Orchestration(
...
fn parse_full_graph_xml(
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Eof) => break,
            Err(e) => {
                return Err(TtdError::Executor(base::error::AlzinaError::Orchestration(
    ...
    async fn resolution_failure_preserves_merged_nodes() {
        ...
        impl AgentExecutor for ResolutionFailExecutor {
            async fn execute(
                ...
                if task == "resolution" {
                    return Err(base::error::AlzinaError::Orchestration(
