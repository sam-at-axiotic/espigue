...
pub fn render_paper_headers_section(inputs: &[ExpertResponse]) -> String {
    ...
    for r in inputs {
        ...
        let tier_tag = match r.provenance.credibility_tier {
            search::CredibilityTier::Unknown => String::new(),
            t => format!(" [{}]", t.label()),
        };
        out.push_str(&format!(
            "- {}{}: {} ({}) — {}\n",
...
fn render_sources_with_tier(
    ...
    sources
        ...
        .map(|s| {
            match tier_map
                .get(s)
                .filter(|t| **t != search::CredibilityTier::Unknown)
            {
...
pub fn render_revision_v2(
    ...
    let tier_map: std::collections::BTreeMap<String, search::CredibilityTier> = panel
        .iter()
