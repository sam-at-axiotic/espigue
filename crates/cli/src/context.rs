...
use base::EmbeddingService;
use orchestration::AgentExecutor;
use search::{JinaRerankService, LitGateway, S2Client, SqliteVecStore};
...
impl LitContext {
    ...
    pub async fn open(cfg: ContextConfig<'_>) -> anyhow::Result<Self> {
        ...
        if !search::schema::register_sqlite_vec_extension() {
        ...
        search::lit_migrate(&lit_pool, cfg.embedding_dim)
            .await
