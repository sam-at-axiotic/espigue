...
//! The lit-store entry point is `alzina_search::lit_fusion::gate_fused` /
...
///
/// Uses `alzina_search::lit_fusion::gate_fused` / `fuse_rrf_three_lane` as
...
/// never imports daemon or `alzina_search` fusion types — the trust boundary
