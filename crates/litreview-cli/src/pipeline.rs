...
use alzina_core::EmbeddingTask;
use alzina_orchestration::{
    ttd::{
...
use alzina_search::{BibliographyStore, FusedHit, SqliteBibliographyStore};
...
impl alzina_orchestration::ttd::engine::PanelRefresher for LitPanelRefresher {
    async fn refresh(
        &self,
        source_ids: &[String],
    ) -> Result<Vec<alzina_orchestration::adapter::ExpertResponse>, String> {
        ...
        let panel = alzina_orchestration::adapter::build_panel(hits, &self.pool)
            .await
...
async fn run_three_lane_fusion(
    ...
    use alzina_core::{VectorFilters, VectorStore};
    ...
    let (
        ...
        async {
            if policy == RetrievalPolicy::LocalOnly {
                ...
                match gateway.acquire(alzina_search::Endpoint::S2).await {
                    alzina_search::Acquire::Proceed => {
                    ...
                    alzina_search::Acquire::BudgetExhausted => (
                        vec![],
        ...
        async {
            if policy == RetrievalPolicy::LocalOnly || warm_internal_lane_enabled() {
                ...
                {
                    Ok(vec_hits) => {
                        let hits: Vec<alzina_core::SearchResultHit> = vec_hits
                            .into_iter()
                            .map(|h| alzina_core::SearchResultHit {
    ...
    let fused = alzina_search::fuse_rrf_three_lane(
        &internal_hits,
        &arxiv_hits,
        &s2_hits,
        alzina_search::RRF_K_DEFAULT,
    );
    let (mut fused, mut degradation_reason) = alzina_search::gate_fused(fused, lane_degraded);
                ...
                match reranker.svc.rerank(query, &docs).await {
                    Ok(scores) => {
                        let (reranked, rerank_reason) =
                            alzina_search::apply_rerank(fused, &scores, reranker.min_score);
                        fused = reranked;
...
fn spawn_fulltext_promotion(
    ...
    for hit in hits.iter() {
        ...
        if hit.source_id.starts_with("s2:") {
            ...
            let embedder_clone_pdf: Arc<dyn alzina_core::EmbeddingService> =
            ...
            promotion_handles.push(tokio::spawn(async move {
                use alzina_search::pdf_fetch::PdfFetchConfig;
                use alzina_search::{
        ...
        let embedder_clone: Arc<dyn alzina_core::EmbeddingService> = Arc::clone(embedder_arc);
        ...
        promotion_handles.push(tokio::spawn(async move {
            use alzina_search::{Acquire, ArxivClient, ArxivConfig, Endpoint, LitChunkConfig};
            ...
            if let Err(e) =
                alzina_search::set_fulltext_status(pool_clone.as_ref(), &paper_id, "pending").await
            {
                tracing::warn!(
                    arxiv_id = %arxiv_id,
                    error = %e,
            ...
            let client = match ArxivClient::new(ArxivConfig::default()) {
                ...
                Err(e) => {
                    ...
                    if let Err(se) = alzina_search::set_fulltext_status(
                        pool_clone.as_ref(),
                        &paper_id,
                        "failed",
                    )
            ...
            let meta = alzina_search::ArxivResult {
            ...
            if let Err(e) = alzina_search::promote_arxiv_fulltext(
                pool_clone.as_ref(),
                store_clone.as_ref(),
...
async fn run_arxiv_lane_for_synthesize(
    query: &str,
    query_vec: &[f32],
    lit_store: &alzina_search::SqliteVecStore,
    embedder: &dyn alzina_core::EmbeddingService,
    lit_pool: Option<&sqlx::SqlitePool>,
    gateway: &Arc<alzina_search::LitGateway>,
) -> (Vec<alzina_search::ArxivHit>, Option<String>) {
    use alzina_core::{VectorFilters, VectorStore};
    use alzina_search::{Acquire, ArxivClient, ArxivConfig, Endpoint};
    ...
    if let Some(pool) = lit_pool {
        ...
        for meta in &arxiv_results {
            ...
            let already = match alzina_search::paper_is_ingested(pool, &paper_id).await {
                Ok(v) => v,
                Err(e) => {
                    tracing::warn!(
                        paper_id = %paper_id,
            ...
            match alzina_search::persist_arxiv_abstract(pool, lit_store, embedder, meta).await {
                Ok(()) => n_ingested += 1,
                Err(e) => tracing::warn!(
    ...
    let hits = vec_hits
        ...
        .map(|h| alzina_search::ArxivHit {
            ...
            content: h.metadata.content_preview.clone(),
            content_preview: alzina_core::truncate_for_preview(&h.metadata.content_preview),
            relevance: h.similarity,
        })
        .collect();
...
async fn run_s2_lane_for_synthesize(
    query: &str,
    s2_client: &alzina_search::S2Client,
    lit_pool: Option<&sqlx::SqlitePool>,
    lit_store: Option<&alzina_search::SqliteVecStore>,
    embedder: Option<&dyn alzina_core::EmbeddingService>,
) -> (Vec<alzina_search::S2Hit>, Option<String>) {
    ...
    let canonical_id = |r: &alzina_search::S2Result| -> String {
        match &r.arxiv_id {
    ...
    if let (Some(pool), Some(store), Some(embed)) = (lit_pool, lit_store, embedder) {
        for r in &s2_results {
            ...
            let persisted = if let Some(aid) = &r.arxiv_id {
                ...
                alzina_search::persist_arxiv_abstract(pool, store, embed, &meta).await
            } else {
                let full = alzina_search::S2PaperFull {
                ...
                alzina_search::persist_s2_abstract(pool, store, embed, &full).await
            };
    ...
    let hits = s2_results
        ...
        .map(|r| {
            ...
            let preview = alzina_core::truncate_for_preview(&body);
            let relevance = r
                .citation_count
...
pub async fn run_review(
    ...
    let (fused_hits, fusion_notice, topicality_notice, seed_notice) = if !opts.seed_papers.is_empty()
    {
        ...
        if opts.scope == Scope::CorpusPlusWeb {
            ...
            let explore_cfg = alzina_search::ExploreConfig::from_env();
            let seed_queries = decompose_seed_queries(&executor, &trimmed, model).await;
            let stats = alzina_search::explore_from_queries(
                &trimmed,
                &seed_queries,
                &explore_cfg,
    ...
    let panel = match alzina_orchestration::adapter::build_panel(fused_hits, ctx.lit_pool.as_ref())
        .await
    {
        Ok(p) => p,
        Err(e) => {
    ...
    let make_callback = |ctx_c: LitContext, top_k_c: usize, policy_c: RetrievalPolicy| {
        CallbackLitSearch::new(move |query: String, _top_k_inner: usize| {
            ...
            async move {
                ...
                let contexts: Vec<alzina_orchestration::ttd::stages::RetrievedContext> = hits
                    .into_iter()
                    .map(|h| alzina_orchestration::ttd::stages::RetrievedContext {
    ...
    let live_retriever: Arc<dyn alzina_orchestration::ttd::retrieval::Retriever> =
        Arc::new(LitRetriever::new(Arc::new(live_search)));

    let local_search = make_callback(ctx.clone(), top_k, RetrievalPolicy::LocalOnly);
    let local_retriever: Arc<dyn alzina_orchestration::ttd::retrieval::Retriever> =
